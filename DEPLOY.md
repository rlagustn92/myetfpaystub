...
(공식 문서: "All apps without traffic for 12 hours go to sleep"). 자는 앱에
들어오면 "Zzzz" 화면이 뜨고, **사람이 깨우기 버튼을 눌러야** 하며 1분쯤
걸립니다. 처음 온 사람에게는 그냥 고장 난 서비스입니다.

그래서 `.github/workflows/keepalive.yml` 이 **4시간마다 주소를 한 번** 부릅니다.

- 주소를 그냥 GET 하면 **HTML 껍데기만** 옵니다. Streamlit 은 브라우저가
  웹소켓을 연결할 때 비로소 `app.py` 를 돌리므로 **시세·분배금 조회는 0회**입니다.
- ⚠ **이미 잠든 앱은 이걸로 못 깨웁니다.** 잠들지 않게 막는 장치입니다.
- ⚠ 쿠키를 저장하며 리다이렉트를 따라가야 합니다(그냥 `-L` 만 주면 무한 루프).
- ⚠ GitHub 은 **60일 동안 커밋이 없는 저장소의 예약 작업을 자동으로 끕니다.**
  오래 쉬었다가 앱이 다시 자기 시작하면 Actions 탭을 먼저 보세요.
- 4시간인 이유: 12시간 안에만 들어오면 되지만 GitHub 예약 실행이 자주 밀립니다.
  한 번 걸러도 괜찮게 여유를 뒀습니다.
